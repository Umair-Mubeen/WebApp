...
def extract_pay_designation_vendor_from_pdf(pdf_path, personnel_excel):
    try:
        # Load personnel Excel file
        df = pd.read_excel(personnel_excel)

        # Add missing columns if not present
        for column in ['Basic Pay', 'PayRoll Designation', 'Vendor Number']:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for idx, personnel in enumerate(personnel_list):
                ...
                basic_pay, designation, vendor_number = None, None, None
                found_pers = False
                ...
                for page in pdf.pages:
                    ...
                    if f"Personnel Number: {personnel}" in text:
                        ...
                        vendor_pattern = r'Vendor Number:\s*(\d+)\s*-\s*([\w\s]+)'
                        vendor_match = re.search(vendor_pattern, text)
                        if vendor_match:
                            vendor_number = vendor_match.group(1)  # Extract only the number
                ...
                df.at[idx, 'Vendor Number'] = vendor_number if vendor_number else 'Not Found'

                print(f'Personnel # {personnel} -> Basic Pay: {df.at[idx, "Basic Pay"]}, '
                      f'Designation: {df.at[idx, "PayRoll Designation"]}, Vendor: {df.at[idx, "Vendor Number"]}')
        ...
        for row in range(2, len(df) + 2):
            for col in range(1, len(df.columns) + 1):
                ...
                if col == df.columns.get_loc('PayRoll Designation') + 1 and cell.value == 'Unknown':
                    cell.fill = fill_style_missing
                if 'Vendor Number' in df.columns and col == df.columns.get_loc('Vendor Number') + 1 and cell.value == 'Not Found':
                    cell.fill = fill_style_missing
...
personnel_excel_path = 'C:/Users/ACS/Documents/res.xlsx'
extract_pay_designation_vendor_from_pdf(pdf_file_path, personnel_excel_path)
